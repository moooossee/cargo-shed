...
pub fn rule(rule_id: &str) -> Result<&'static str, ShedError> {
    match rule_id {
        "tokio-full" => Ok(
            "tokio-full\n\nDetects tokio dependencies that enable the full feature set. cargo-shed reports this because full often pulls in more compile-time cost than the project actually needs. A future safe fix will replace full only when source usage gives enough evidence.",
        ),
        ...
        "unused-dependency" => Ok(
            "unused-dependency\n\nDetects declared dependencies that do not appear in scanned Rust source files. This is heuristic, so cargo-shed is conservative before suggesting a removal.",
        ),
